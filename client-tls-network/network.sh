...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    #docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf system-genesis-block/*.block organizations/peerOrganizations organizations/ordererOrganizations system-genesis-block'
    ...
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf channel-artifacts log.txt fabcar.tar.gz fabcar examplecc.tar.gz'
...
COMPOSE_FILE_BASE=docker/docker-compose-client-net.yaml
