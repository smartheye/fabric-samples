...
const Chaincode = class {
	...
	async Invoke(stub) {
		...
		try {
			...
			// add for test
			if (fcn === 'set1') {
				return this.set1(stub, args);
			}
		} catch (error) {
			return shim.error(error.toString());
		}

		logger.error(`Unknown action, check the first argument, must be one of 'delete', 'query', 'move', 'init'. But got: ${fcn}`);
		return shim.error(`Unknown action, check the first argument, must be one of 'delete', 'query', 'move', 'init'. But got: ${fcn}`);
	}
	...
	async set1(stub, args) {
		logger.info('########### example_cc0 set1 ###########');
		...
		try {
			...
			return shim.success(Buffer.from(`"${A}":"${B}"`));
		} catch (e) {
			return shim.error(e);
		}
